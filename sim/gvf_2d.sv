...
module gvf_2d;
    ...
    localparam NUM_CORES         = 4;    // <-- 2x2x1 = 4
    ...
    // Kernel FSM States (identical to gvf.sv)
    ...
    // LFSR for randomization (same seed as gvf.sv for determinism)
    reg [31:0] lfsr = 32'hDEADBEEF;
    always @(posedge clk)
        lfsr <= {lfsr[30:0], lfsr[31] ^ lfsr[21] ^ lfsr[1] ^ lfsr[0]};

    // INFRASTRUCTURE TASKS (identical to gvf.sv)
    ...
    // ISA PROGRAM LIBRARY (identical to gvf.sv - same stimulus)
    ...
    task automatic load_prog_all_opcodes;
        write_pmem(0,  enc_const(4'h0, 8'h05));
        write_pmem(1,  enc_const(4'h1, 8'h03));
        write_pmem(2,  {OP_NOP, 12'h000});
        write_pmem(3,  enc_rrr(OP_ADD, 4'h2, 4'h0, 4'h1));
        write_pmem(4,  enc_rrr(OP_SUB, 4'h3, 4'h0, 4'h1));
        write_pmem(5,  enc_rrr(OP_MUL, 4'h4, 4'h0, 4'h1));
        write_pmem(6,  enc_rrr(OP_DIV, 4'h5, 4'h0, 4'h1));
        write_pmem(7,  enc_rrr(OP_CMP, 4'h0, 4'h2, 4'h3));
        write_pmem(8,  {OP_STR, 4'h2, 4'h0, 4'h0});
        write_pmem(9,  {OP_LDR, 4'h6, 4'h0, 4'h0});
        write_pmem(10, enc_ret());
        repeat(2) @(posedge clk);
    endtask

    // Memory read-after-write program
    ...
    // KERNEL RUNNER - tagged with CONFIG_LABEL
    task automatic run_kernel(
        ...
        $display("[CSV] %0s,%0s,%0d,%0d,%0d,%0d,%0d",
                 CONFIG_LABEL, name, threads, t_end - t_start,
    ...
    // CONTINUOUS MONITORS (adapted for 4-core topology)
    ...
    // M9: Kernel Eventually Completes
    ...
    // MAIN TEST SEQUENCE (identical stimulus to gvf.sv)
    initial begin
        integer i, j, k;
        ...
        // Note: dbg_core_done_sample is 8 bits, upper 4 will be 0 at CUBE_Z=1
        gvf_check(dbg_core_done_sample[NUM_CORES-1:0] == {NUM_CORES{1'b0}}, "all cores idle");
        ...
        // 32T requires 8 blocks on 4 cores - now works with scheduler fix
        // (first-wave dispatch + recycling during RUNNING)
        do_reset(); load_prog_saxpy();
